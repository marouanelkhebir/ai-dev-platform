...
This is a Java 21 / Spring Boot 3.3 Maven service. Production code is under `src/main/java/com/company/aidev/`; packages map to platform concerns: `api/` for HTTP endpoints, `workflow/` for the persisted state machine, `agent/` and `tool/` for LLM agents, `persistence/` for JPA, and `security/` and `sandbox/` for enforcement. Configuration and Flyway migrations live in `src/main/resources/`; test configuration is `src/test/resources/application-test.yml`.
...
Write JUnit 5 tests beside the matching package. Name fast tests `*Test` and infrastructure, HTTP, or database tests `*IT`; Failsafe runs the latter during `verify`. Use WireMock for external Jira/GitLab contracts, H2 for fast contexts, and Testcontainers PostgreSQL when migration behavior matters. Add regression coverage for workflow transitions, command/path guards, and secret handling.
