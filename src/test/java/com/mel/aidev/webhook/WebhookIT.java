...
import com.mel.aidev.persistence.repository.WebhookEventRepository;
...
class WebhookIT {
    ...
    void setUp() {
        ...
        // A ticket now reaches a project rather than a bare repository path. The webhook resolves it
        // from the Jira key, which is why the project declares BANK.
        ProjectEntity project = new ProjectEntity(java.util.UUID.randomUUID(), "Banque", PROJECT);
        project.setJiraProjectKey("BANK");
        projectRepository.save(project);

        when(gitLabClient.getProject(PROJECT))
                .thenReturn(new GitLabProject(1L, "cm", PROJECT, "main", "https://gitlab/x", "https://gitlab/x.git"));
    }
    ...
    /**
     * The Jira key is what maps a ticket to a project now; a key no project declares, and no label
     * to fall back on, leaves nothing to resolve.
     */
    @Test
    @DisplayName("a labelled ticket of an unknown Jira project is not started")
    void shouldRefuseWithoutProjectLabel() throws Exception {
        mockMvc.perform(post("/webhooks/jira")
                        .header("X-Webhook-Token", JIRA_SECRET)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(jiraPayload("OTHER-1247", "agent-ready", "team-payments", 3)))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.status").value("ignored"));
