...
public class BranchPolicy {
    ...
    // ------------------------------------------------------------------ per project
    //
    // A project may use its own prefix and protect more branches than the platform does. It can
    // never protect fewer: the global list stays a floor, so a project form is not a way to
    // unprotect main.

    public String branchFor(ProjectConfiguration configuration, String jiraTicket) {
        return configuration == null ? branchFor(jiraTicket) : configuration.branchFor(jiraTicket);
    }

    public boolean isAgentBranch(ProjectConfiguration configuration, String branch) {
        if (configuration == null) {
            return isAgentBranch(branch);
        }
        return branch != null && branch.startsWith(configuration.branchPrefix());
    }

    public boolean isProtected(ProjectConfiguration configuration, String branch) {
        if (isProtected(branch)) {
            return true;
        }
        if (configuration == null || branch == null) {
            return false;
        }
        String normalized = branch.toLowerCase(Locale.ROOT);
        return configuration.protectedBranches().stream().anyMatch(protectedBranch -> {
            String candidate = protectedBranch.toLowerCase(Locale.ROOT);
            return normalized.equals(candidate) || normalized.startsWith(candidate + "/");
        });
    }
    ...
    /**
     * @throws BranchPolicyViolationException when the branch does not carry the project prefix
     */
    public void assertAgentBranch(ProjectConfiguration configuration, String branch) {
        if (configuration == null) {
            assertAgentBranch(branch);
            return;
        }
        if (!isAgentBranch(configuration, branch)) {
            throw new BranchPolicyViolationException("Agents may only write on branches prefixed with '"
                    + configuration.branchPrefix() + "', got: " + branch);
        }
    }

    /**
     * @throws BranchPolicyViolationException when the branch is protected, globally or by the project
     */
    public void assertNotProtected(ProjectConfiguration configuration, String branch) {
        if (isProtected(configuration, branch)) {
