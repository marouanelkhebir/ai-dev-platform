...
public class WorkflowEngine {
    ...
    private static final String AI_PROCESSED_LABEL = "traitee-par-ia";
    ...
    public void advance(UUID workflowId) {
        ...
        try {
            ...
            ProjectConfiguration configuration = launchConfiguration.of(claimed.get());
            WorkflowRun run = new WorkflowRun(claimed.get(), configuration);
            try (var ignored = LogContext.of()
                            ...
                            .apply();
                    // Opened for the whole run: the model provider is called from inside LangChain4j,
                    // the LLM audit listener is a callback, and the build tools are created per agent
                    // execution. None of them can receive the project as an argument.
                    var ignoredProjectContext = ProjectRuntimeContext.open(configuration)) {
                runSteps(run);
            } finally {
                destroySandbox(run);
    ...
    private StepOutcome analyzeJira(WorkflowRun run) {
        ...
        // The base branch comes from the frozen configuration, not from a fresh GitLab lookup: a
        // repository that changes its default branch mid-workflow must not move the target of a
        // merge request that is already open.
        String baseBranch = run.configuration().baseBranch();
        ...
        if (needsClarification) {
            String reason = "The ticket contains ambiguities that must be resolved by a human.";
            if (workflow.isJiraBacked()) {
                commentJiraSafely(
                ...
                transitionJiraSafely(workflow.getJiraTicket(), settings.jira().statuses().needsClarification());
            }
            return StepOutcome.to(WorkflowStatus.NEEDS_CLARIFICATION, reason);
        }

        if (workflow.isJiraBacked()) {
            transitionJiraSafely(workflow.getJiraTicket(), settings.jira().statuses().inProgress());
        }
        return StepOutcome.to(
    ...
    private StepOutcome securityReview(WorkflowRun run) {
        ...
        var scannerReports = workflow.getPipelineId() == null
                ? List.<com.mel.aidev.gitlab.model.ScannerReport>of()
    ...
    private StepOutcome acceptance(WorkflowRun run) {
        ...
        publishFinalReport(workflow, analysis, testReport);
        if (workflow.isJiraBacked()) {
            transitionJiraSafely(workflow.getJiraTicket(), settings.jira().statuses().readyForReview());
            addAiProcessedLabelSafely(workflow.getJiraTicket());
        }
        return StepOutcome.to(
    ...
    private com.mel.aidev.domain.RepositoryRules loadRules(WorkflowRun run) {
    ...
    private void ensureSandbox(WorkflowRun run) {
        ...
        WorkflowEntity workflow = run.workflow();
        ProjectConfiguration configuration = run.configuration();
        GitLabProject project = gitLabClient.getProject(workflow.getGitlabProject());

        if (workflow.getBranch() == null) {
            workflow.setBranch(branchPolicy.branchFor(configuration, workflow.getJiraTicket()));
        }
        branchPolicy.assertAgentBranch(configuration, workflow.getBranch());
        branchPolicy.assertNotProtected(configuration, workflow.getBranch());
        ...
        // The project image wins; without one, the administrator-configured image of the detected
        // profile applies. Either way the value actually used is recorded on the workflow, so the
        // audit says which image ran the code rather than which one the configuration mentions today.
        String image = configuration.sandboxImage() == null
                ? sandboxProperties.imageFor(buildProfile)
                : configuration.sandboxImage();
        workflow.setSandboxImage(image);

        Sandbox sandbox = sandboxManager.createSandbox(
                workflow.getId(), workflow.getJiraTicket(), buildProfile, image, configuration.variables());
        run.sandbox(sandbox);
    ...
    private void applyOutcome(WorkflowRun run, StepOutcome outcome) {
        ...
        if (outcome.nextStatus() == WorkflowStatus.FAILED) {
            ...
            if (workflow.isJiraBacked()) {
                transitionJiraSafely(workflow.getJiraTicket(), settings.jira().statuses().failed());
                commentJiraSafely(
    ...
    private void addAiProcessedLabelSafely(String ticket) {
        try {
            jiraClient.addLabel(ticket, AI_PROCESSED_LABEL);
        } catch (RuntimeException e) {
            log.warn("Unable to add AI processed label to Jira issue {}: {}", ticket, e.toString());
        }
    }
